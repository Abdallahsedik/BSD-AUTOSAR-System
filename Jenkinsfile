        ...
        stage('Unit Tests') {
            steps {
                ...
                bat '''
                    ...
                    gcc ^
                     ...
                     gcc ^
                            ...
                            echo Building Warning Output Tests...
                            gcc ^
                                -I "%WORKSPACE%\\%UNITY_DIR%" ^
                                -I "%WORKSPACE%\\%RTE_DIR%" ^
                                -I "%WORKSPACE%\\%SWC_CODE_DIR%" ^
                                -I "%WORKSPACE%\\%EB_INCLUDE%" ^
                                "%WORKSPACE%\\%UNITY_DIR%\\unity.c" ^
                                "%WORKSPACE%\\%TEST_DIR%\\test_BSD_Warning_SWC.c" ^
                                "%WORKSPACE%\\%SWC_CODE_DIR%\\BSD_Warning_SWC.c" ^
                                -o "%WORKSPACE%\\%REPORT_DIR%\\test_runner_warning.exe" ^
                                || exit 1

                            echo Building Diagnostic Tests...
                            gcc -I "%WORKSPACE%\\%UNITY_DIR%" -I "%WORKSPACE%\\%RTE_DIR%" ^
                                -I "%WORKSPACE%\\%SWC_CODE_DIR%" -I "%WORKSPACE%\\%EB_INCLUDE%" ^
                                "%WORKSPACE%\\%UNITY_DIR%\\unity.c" ^
                                "%WORKSPACE%\\%TEST_DIR%\\test_BSD_Diagnostic_SWC.c" ^
                                "%WORKSPACE%\\%SWC_CODE_DIR%\\BSD_Diagnostic_SWC.c" ^
                                -o "%WORKSPACE%\\%REPORT_DIR%\\test_runner_diagnostic.exe" || exit 1
                '''
                ...
                bat '''
                    ...
                    echo Running  Warning Output Tests...
                    "%WORKSPACE%\\%REPORT_DIR%\\test_runner_warning.exe" >> "%WORKSPACE%\\%REPORT_DIR%\\test_output.txt" 2>&1
                    
                    echo Running  Diagnostic Tests...
                    "%WORKSPACE%\\%REPORT_DIR%\\test_runner_diagnostic.exe" >> "%WORKSPACE%\\%REPORT_DIR%\\test_output.txt" 2>&1
